    ...
    useEffect(() => {
        const filterData = () => {
            if (selectedMonth === '') {
                ...
                setTotalDistributed(charges.reduce((sum, item) => sum + parseFloat(item.amount || 0), 0));
            } else {
                ...
                setTotalDistributed(filtered.reduce((sum, item) => sum + parseFloat(item.amount || 0), 0));
            }
        };
        filterData();
                ...
                <div className="bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-100">
                    <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-gray-50">
                            <tr>
                                {['SC ID', 'Employee', 'Role', 'Event', 'Event Budget', 'Amount', 'Date', 'Customer'].map((header) => (
                                    <th 
                                        key={header}
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {filteredData.map((row) => (
                                ...
                                >
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        {new Date(row.calculation_date).toLocaleDateString('en-GB', {
                                            day: 'numeric',
                                            month: 'short',
                                            year: 'numeric'
                                        })}
